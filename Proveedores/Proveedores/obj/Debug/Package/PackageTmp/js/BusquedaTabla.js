...
function obtenerPARAMS(Zzvalor) {
    var arruuid = [];
    $('.chkuuid:checked').each(
        function () {
            arruuid.push($(this).attr("id"));
    ...
    if (arruuid.length > 0) {
        //alert('voy a desadju ' + Zzvalor);
        desadjuFV60XML(arruuid.toString(), Zzvalor.toString())
    }
}

function desadjuFV60XML(uuid, Zzvalor) {
    var params = new Object();
        params.uuid = uuid + "," + Zzvalor;
        params = JSON.stringify(params);
//        alert('mando a desajufv60 ' + params);
    $.ajax({
        type: "POST",
        url: "../portal/factFV60.aspx/desadjuFV60XML",
        data: params,
...
function mostrarFV60(result) {
    var dialog;
    alert('llego a mostrarfv60 ');
...
function recargarFV60() {       //Create a Form
    alert(' Recargarfv60 ');
    var $form = $("<form/>").attr("id", "data_form")
        .attr("action", "factFV60.aspx")
        .attr("method", "post");
    $("body").append($form);
